...
ml/llm.py
=========
Provider-agnostic LLM client for the two places DemandPulse uses one:
...
key → a reachable Ollama server. Override with DEMANDPULSE_LLM_PROVIDER.
...
DEMANDPULSE_OLLAMA_URL at a host the app can actually reach.
...
def ollama_url() -> str:
    return (_secret("DEMANDPULSE_OLLAMA_URL") or _secret("OLLAMA_HOST")
...
def provider() -> str:
    ...
    explicit = bool(os.environ.get("DEMANDPULSE_LLM_PROVIDER") or
                    _secret("DEMANDPULSE_LLM_PROVIDER"))
...
def model_name(p: str = None) -> str:
    p = p or provider()
    return (_secret("DEMANDPULSE_LLM_MODEL") or LLM_MODEL or DEFAULT_MODELS[p])
...
def status() -> dict:
    ...
    if p == "ollama":
        ...
        out["hint"] = (f"Set {key} in the environment or Streamlit secrets — or run "
                       ...
                       f"(DEMANDPULSE_LLM_PROVIDER=ollama).")
    out["options"] = {
        "ollama": "free, local, no key — ollama.com, then pull qwen2.5:7b",
        "openai": "export OPENAI_API_KEY and DEMANDPULSE_LLM_MODEL=gpt-4o",
